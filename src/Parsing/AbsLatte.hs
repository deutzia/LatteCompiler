...
data TopDef a
    ...
    | ClassExtDef a Ident Ident [ClassBody a]
  deriving (Eq, Ord, Show, Read)
...
instance Functor TopDef where
    fmap f x = case x of
        ...
        ClassExtDef a ident1 ident2 classbodys -> ClassExtDef (f a) ident1 ident2 (map (fmap f) classbodys)
data ClassBody a = AttrFun a (FunDef a) | Attr a (Type a) Ident
  deriving (Eq, Ord, Show, Read)

instance Functor ClassBody where
    fmap f x = case x of
        AttrFun a fundef -> AttrFun (f a) (fmap f fundef)
...
data BuiltinType a = Int a | Str a | Bool a | Void a
  deriving (Eq, Ord, Show, Read)
...
instance Functor ArrayType where
    fmap f x = case x of
        BuiltinArr a builtintype -> BuiltinArr (f a) (fmap f builtintype)
        UserArr a ident -> UserArr (f a) ident
data Type a
