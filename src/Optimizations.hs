...
lcse :: [([Q.Block], [String])] -> [([Q.Block], [String])]
lcse = map (\(blocks, args) ->
    let blocks' = map lcseSingleBlock blocks in
...
canonicalForm _ q@(Q.AssignLocal _ _) = q
            ...
            case quad' of
                ...
                Q.GetVar r vName -> case M.lookup vName vMap of
                    Just oldRes ->
                        (M.insert r oldRes subs, qMap, vMap, ptrMap, acc)
                    Nothing ->
                        (subs, qMap, M.insert vName r vMap, ptrMap, quad' : acc)
                Q.AssignVar vName r ->
                        (subs, qMap, M.insert vName r vMap, ptrMap, quad' : acc)
                Q.AssignLocal r n ->
                        (M.insert r (Q.Literal n) subs,
